...
\begin{table}[H]
	...
	\begin{tabular}{l@{\qquad}cccc}
		...
		Fork & $S_{F}= 200\ \mm^2$ & $l_{F}=170\ \mm$ & Aluminium & 1 \\[5em]
		...
		Bogie 3 & $S_{B3}=201 \ \mm^2$ & $t_{B3}=37\ \mm$ & Aluminium & 1 \\		
	\end{tabular}
\end{table}	
...
\begin{table}[H]
	...
	\begin{tabular}{l@{\qquad}cccc}
		...
		& & & & \\[-0.75em]
		 ...
		 Bolts & $S_{B}= 3.14 \ \mm^2$ & $t_{B}= 18\ \mm$ & Steel & 4 \\[5em]
		\hline
	Name &  Linked Components &  \multicolumn{3}{c}{Geometry}\\ \hline
	& & & & \\[-0.5em]
	...
	Fork 1& $S_{F2}=200 \ \mm^2$ & $l_{F2}=175$ & Aluminium & 1 \\[3em]	
\end{tabular}
\end{table}	
...
\begin{table}[H]
	...
	\begin{tabular}{l@{\qquad}cccc}
		...
		Name &  Linked Components &  \multicolumn{3}{c}{Geometry}\\ \hline
		& & & & \\[-0.5em]
		\textbf{Rim} & \textbf{Node$_2$} $\leftrightarrow$ \textbf{Ground}  &  \multicolumn{3}{c}{\multirow{2}{*}{\adjustbox{valign=t}{\includegraphics[width=0.255\textwidth]{Media/tcs_con_rim}}}}  \\[1em]
		& & & &  \\
		\multicolumn{2}{l}{$CON_{13}= 5.11 \cdot 10^{-3}\ \frac{\W}{\K}$} & & & \\[9em]
...
\begin{table}[H]
	...
	\begin{tabular}{c@{\quad}ccc}
		...
		Heat switch name  & Linked Components & Temperature  & Amount \\ \hline
		 ...
		 $CON_{S3}$ & {Drive Engine} $\leftrightarrow$ {Node$_2$} & -20\ $^\circ$C  & $n_{S3}=1$ (in total 4)  \\[1em]\hline
	\end{tabular}
