...
\multirow{5}{*}{Structure \& Mechanism} & Harness                             & 3                           & 1        & 20\%             & 3.60           \\
                                        & Chassis                              & look up at Chassis Table     & 1        & 20\%             & 5.02           \\
...
\begin{table}[hbt]
\centering
\caption{INSPIRE chassis mass budget.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llllll@{}}
\toprule
\multicolumn{2}{c}{Component  Name}                  & \multicolumn{1}{c}{Mass per  Component / kg} & \multicolumn{1}{c}{Quantity} & \multicolumn{1}{c}{Component Margin} & \multicolumn{1}{c}{Total / kg} \\ \midrule
\multicolumn{2}{c}{Cam Head Box}                     & \multicolumn{1}{c}{0.223}                        & \multicolumn{1}{c}{1}        & \multicolumn{1}{c}{20\%}             & \multicolumn{1}{c}{0.27}           \\
...
\begin{figure}[htb] 
  ...
  \caption{Mean maximum pressure plotted over the deflection \(\delta\). The maximum value of 40~kPa is only exceeded for very small values of \(\delta\).}
  \label{fig:MMP}
\end{figure}
...
\begin{table}[htb]
\centering
\caption{Calculated torque and power of the configuration 6 in respect to the slope angle and velocity.}
\begin{adjustbox}{max width=\textwidth}
...
\autoref{eqn:EbzuN0} and the conservative assumptions from \autoref{tab:lb-param} results in an energy per bit over noise $\frac{{E}_{b}}{{N}_{0}} = 10.59\ dB$. The complete link budget can be found in \autoref{tab:lb-R2L}. \\
...
Using the same parameters listed in \autoref{tab:lb-param} leads to a $\frac{{E}_{b}}{{N}_{0}} = 50.56\ dB$ corresponding to a Bit Error Rate of potentially less than $10^{-6}$. Therefore the downlink does not contribute to the design decisions. The complete link budget for the uplink path can be found in \autoref{tab:lb-L2R}.
...
In a first assumption 420 images per day are stored and transmitted. Considering the sensor resolution of $2048\times2048$ pixels and a bit depth per pixel of 10 bit, a file size of 42 Mbit per image is assumed (camera datasheet in \autoref{app:DigitalAppendix}).\\
...
\begin{tabular}{llllll}
\toprule
File Type & Compression    & File Size  & Message Size & Data Rate & Tx t per File \\ 
\midrule
image     & none           & 42 Mbit    & 48.77 Mbit   & 5 Mbit    & 8.12 s        \\
image     & HIREW          & 23.52 Mbit & 27.31 Mbit   & 5 Mbit    & 5.45 s        \\
\bottomrule
\end{tabular}
...
\begin{tabular}{lllll}
\toprule
File per tal & Compression & File Size  & Total Data & Tx time    \\
\midrule
420          & none        & 42 Mbit    & 2.21 GB    & 56.9 Min.  \\
420          & HIREW       & 23.52 Mbit & 1.22 GB    & 38.23 Min. \\
\bottomrule
\end{tabular}
\label{tab:Tx-tptal}
\end{table}
...
\begin{table}[htb]
	...
	\toprule
		Name  & Path & Type & Size  \\
	\midrule
		TCS Analysis final & 300 Thermal\ & Excle Sheet with Macros, *.xlsm & 189 kB  \\
		Comms \& C\&DH Datasheets & 600\_C\&DH\_Communication & *.pdf & 42.5 MB\\
		Comms \& C\&DH Trade Offs &	600\_C\&DH\_Communication & *.png & 479 kB\\
				Locomotion MMP Deflection 			& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 101 kB  \\	
		...
		SinkageSnow 						& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 4 kB  \\
				SinkageClay				 			& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 4 kB  \\	
		
		Torque and Power 					& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 11 kB  \\
		
		WheelDimensionsV3 reduced			& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 11 kB  \\
		
		WheelDimensionsV3 groß				& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 12 kB  \\
		Tradeoff Locomotion				& 200 Locomotion\ & Excle Sheet with Data, *.xlsx	& 475 kB  \\
			
Lens Assembly		& 700 Payload\ & PDF Datasheet, *.pdf	& 353 kB  \\	

RadHard Lenses Schott 					& 700 Payload\ & PDF Datasheet, *.pdf	& 338 kB  \\	

PowerLED		& 700 Payload\ & PDF Datasheet, *.pdf	& 843 kB  \\

SpaceGradeCameraCMOS		& 700 Payload\ & PDF Datasheet, *.pdf	& 869 kB  \\
SunSensorSpaceGradeMAUS		& 700 Payload\ & PDF Datasheet, *.pdf	& 487 kB  \\
	\bottomrule
	\end{tabular}
	\end{adjustbox}
