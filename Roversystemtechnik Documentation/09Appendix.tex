...
\autoref{eqn:EbzuN0} and the conservative assumptions from \autoref{tab:lb-param} results in an Energy per bit over Noise $\frac{{E}_{b}}{{N}_{0}} = 10,59\ dB$. Referencing \autoref{fig:BitErrorRate} a Bit Error Rate of $10^{-4}$ can be achieved which is considered sufficient for a first assessment. Optionally a FEC code can be implemented in later design phases.  

\begin{figure}[hbt]
	\centering
  		\includegraphics[width=1\textwidth]{Media/LB-RovertoLander.png}
...
\begin{figure}[hbt]
	\centering
  		\includegraphics{Media/Bit_Error_Rate.png}
