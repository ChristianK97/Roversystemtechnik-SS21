...
\begin{figure}[htb]
{\centering
\includegraphics[width=0.7\textwidth]{Media/INSPIRE_Locomotion}
\caption{Ackerman Steering Ilustration}
\label{fig:Ackerman}
}
\end{figure}
...
It was assumed, that the toggle temperatur can be adapted by modifying the disk height.
...
Following simplifications and asumptions for the analysis were made.
\begin{itemize}
	...
	\item For the bogies  and the wheel fork radiation was defined. The radiation temperature is the mean temperature of the two connecting nodes. Half of the heat loss was added to corresponding node (either Chassis, Node$_1$ or Node$_2$).
\end{itemize}
...
All temperatures lay betwenn their limits without using a heater.
The margins for uncertainties, acceptance tests and qulification tests were considered with 5 K each, $\pm$15 K in total, \cite{ref_tcs_05}.
...
The issue can be solved by optimising the heat paths and adding furhter insulation during a detailed ebay design process.
