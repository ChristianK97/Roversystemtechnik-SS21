...
The rover is stowed in the lander as in \autoref{fig:EndStorageConfig} and must first place its wheels parallel to the chassis. Then the bogies are rotated by at least 15 degrees so that the Chassi lifts slightly from the lander. As soon as this has happened, a mechanism is triggered which unfolds the ramp. Now INSPIRE can carefully roll out of the lander until it finally reaches the surface of Europa.
...
BAE Systems provides a SBC with their flight proven Power PC750 Architecture which withstands a total radiation dose of up to $1~\text{Mrad}$. The robustness comes at the cost of a $182~\text{MHz}$ processor speed (datasheet in \autoref{app:DigitalAppendix}). \\
