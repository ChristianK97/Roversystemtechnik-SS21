...
\begin{figure}[H]
	...
	\caption{Change of the heat switch conductivity $R_\text{t}$ over the mean temperature $T_\text{M}$.}
...
\begin{itemize}
	...
	\item No discrete nodes for the radar, hazcams, APXS and deployment engines were considered. Their heat will be lead into the chassis.
...
The thermal analysis was performed as an Excel calculation with the  possibility to adapt input values and dimension..
...
The temperature margins for uncertainties, acceptance tests and qualification tests were considered with 5 K each, $\pm$15 K in total, \cite{ref_tcs_05}.
...
The issue can be solved by optimising the heat paths and adding further insulation during the detailed E-Bay design process.
...
\begin{table}[htb]
	...
	\begin{tabular}{lccccc}
		\toprule
		Components & Hot Case & Cold Case & Charging & Communication & Analyse  \\
		\midrule
		RTG  &  &  &  &   &     \\
		Electric Bay &  &  &  &  &  \\
		Drill \& Analyser &  &  &  &  &   \\
