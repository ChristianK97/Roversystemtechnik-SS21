...
The data output is based on a [missing ref] h European day (tal). Optical references are used for navigation which limits operation to [Europa daylight] h of sunlight.
Due to the limited processor speed of the OBC, a path planning time of 5 minutes is estimated. The path planner generates a traversability map for 1 m. Fully charged the rover can execute 70 locomotion legs before switching to the charging mode. In total the rover is able to cover a distance of 140 m per tal.  
...
The data output can be increased further in the case of a sufficient power margin during operation.
